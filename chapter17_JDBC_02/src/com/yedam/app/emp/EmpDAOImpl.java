...
public class EmpDAOImpl extends DAO implements EmpDAO {
	...
	public List<EmpVO> selectAll() {
		...
		try {
			...
			String sql = "SELECT * FROM employees" ; //모든 필드는 * 를 사용하면 된다.
			rs = stmt.executeQuery(sql);
			int count = 0;
			while(rs.next()) {
				EmpVO empVO = new EmpVO();
				empVO.setEmpNo(rs.getInt("emp_no"));
				empVO.setBrithDate(rs.getString("birth_date"));
	...
	public EmpVO selectOne(EmpVO empVO) {
		...
		try {
			...
			String sql = "SELECT * FROM emplotees WHERE emp_no = " + empVO.getEmpNo();
			rs = stmt.executeQuery(sql);
			
			if(rs.next()) {
				findVO = new EmpVO();
				findVO.setEmpNo(rs.getInt("emp_no"));
				findVO.setBrithDate(rs.getString("birth_date"));
	...
	public void insert(EmpVO empVO) {
		try {
			...
			pstmt.setString(2, empVO.getBrithDate());
